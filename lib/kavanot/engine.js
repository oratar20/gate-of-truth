...
// שיטת "ע"ב שמות" (72shemot): מהזריחה המדויקת, כל שם שולט 20 דקות,
// 72 שמות × 20 דק' = 24 שעות. מחזיר את אינדקס השם השולט ברגע.
function compute72(now, loc, tzid) {
  try {
    const t = now.getTime();
    const today = localYMD(now, tzid);
    const zFor = (n) => new Zmanim(loc, ymdNoon(today, n), false);
    let sunrise = zFor(0).neitzHaChama().getTime();
    if (!Number.isFinite(sunrise)) return null;
    // לפני זריחת היום → עוגן לזריחת אתמול (המחזור האחרון)
    if (t < sunrise) {
      const y = zFor(-1).neitzHaChama().getTime();
      if (Number.isFinite(y)) sunrise = y;
    }
    const SLOT = 20 * 60000;
    let idx = Math.floor((t - sunrise) / SLOT);
    idx = ((idx % 72) + 72) % 72;            // 0..71
    const slotStart = sunrise + idx * SLOT;
    return {
      index: idx,                            // 0..71
      num: idx + 1,                          // מספר השם 1..72
      sunrise: new Date(sunrise),
      slotStart: new Date(slotStart),
      slotEnd: new Date(slotStart + SLOT),
    };
  } catch { return null; }
}
...
function computeShaaZmanit(now, loc, tzid) {
  try {
    ...
    // קווי-רוחב קיצוניים (שמש-חצות / לילה-קוטבי): אין זריחה/שקיעה תקינה.
    ...
    // הגנה מפני זמנים סמוכים פגומים (אתמול/מחר בקווי-רוחב קיצוניים)
    if (!Number.isFinite(segLen) || segLen <= 0 || !Number.isFinite(idx)) return null;
    ...
    if (!planet || !PLANET_META[planet]) return null;    // אין התאמה תקינה
    return {
      portion,                       // 'day' | 'night'
