...
import { meaningOf, MEANINGS_72_SOURCE } from './names72_meanings.js';
...
function focusName(state, lang = 'he') {
  ...
  return {
    kind: 'name',
    label: 'שם מע"ב',
    title: [...name].join('.'),          // כתיב מנוקד כמו במקור: י.ז.ל
...
export function buildFoci(state, lang = 'he') {
  return {
    ruler: focusRuler(state),
    yichud: focusYichud(state),
    name: focusName(state, lang),
...
export function pickForMoment(state, lang = 'he') {
  const foci = buildFoci(state, lang);
  // המוקד הראשי = שם ע"ב לפי שיטת 72shemot (זריחה + סלוט 20 דק'), מוצג ראשון.
  // שם מע"ב תמיד קיים (נופל לרוטציה אם אין זריחה). השעה-הזמנית משנית.
  const options = [foci.name, foci.ruler, foci.yichud, foci.havaya, foci.kodesh, foci.ana].filter(Boolean);
  const primary = foci.name;
